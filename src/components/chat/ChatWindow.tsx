...
import { SendIcon, MoreVertIcon, BackArrowIcon, PaperclipIcon } from '../shared/Icons';
...
const ChatWindow: React.FC<ChatWindowProps> = ({ activeConversation, currentUser, messages, onSendMessage, onBack, onBackToDashboard, onViewProfile, onBlockUser, onReportUser, isTyping, onTyping, socket }) => {
    ...
    // Ensure the messages container scrolls to bottom on input focus (mobile keyboard open)
    useEffect(() => {
        const inputEl = document.querySelector('input[type="text"]');
        const onFocus = () => setTimeout(() => messagesContainerRef.current?.scrollTo({ top: messagesContainerRef.current.scrollHeight, behavior: 'smooth' }), 300);
        const onResize = () => setTimeout(() => messagesContainerRef.current?.scrollTo({ top: messagesContainerRef.current.scrollHeight, behavior: 'smooth' }), 300);
    ...
    return (
        <div className="flex flex-col w-full min-h-0 bg-gradient-to-b from-slate-50 to-white relative overflow-hidden">
            ...
            <div className="relative flex-1 flex flex-col p-2 sm:p-4 overflow-hidden">
                {/* Subtle decorative orbs (pointer-events-none) */}
                ...
                <div
                    ...
                    className="relative flex-1 overflow-y-auto custom-scrollbar px-3 py-4 space-y-3 sm:space-y-4"
                    onTouchStart={() => { /* enable mobile touch scrolling */ }}
                >
                    {/* Additional decorative orbs inside the scroll layer (visual only) */}
                    <div className="absolute top-20 left-10 w-72 h-72 bg-gradient-to-br from-purple-200/30 to-pink-200/30 rounded-full filter blur-3xl opacity-50 animate-blob"></div>
                    <div className="absolute bottom-20 right-10 w-72 h-72 bg-gradient-to-br from-blue-200/30 to-indigo-200/30 rounded-full filter blur-3xl opacity-50 animate-blob animation-delay-2000"></div>
                        ...
                        {filteredMessages.map((msg) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    {/* Message content (edit mode or bubble) */}
                                    {isEditing ? (
                                        <div className={`flex ${isSender ? 'justify-end' : 'justify-start'}`}>
                                            <div className="max-w-[70%] bg-white rounded-2xl shadow-xl border-2 border-indigo-400 p-4 space-y-3">
                                                ...
                                                <div className="flex justify-end space-x-2">
                                                    <button onClick={handleCancelEdit} className="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors">Cancel</button>
                                                    <button onClick={handleSaveEdit} className="px-4 py-2 text-sm font-medium text-white bg-gradient-to-r from-indigo-500 to-purple-500 rounded-lg hover:from-indigo-600 hover:to-purple-600 transition-all">Save</button>
                                                </div>
                                            </div>
                                        </div>
                                    ) : (
                ...
                {/* Context Menu (placed outside the scrollable area for proper positioning) */}
                {contextMenu && (
                    <>
                        <div className="fixed inset-0 z-40 md:hidden" onClick={(e) => { e.stopPropagation(); setContextMenu(null); }} />

                        <div
                            className={`fixed rounded-2xl shadow-2xl py-2 min-w-[220px] z-50 animate-fadeIn overflow-hidden ${
                                isAdmin ? 'bg-gradient-to-br from-amber-50 via-white to-amber-50 border-2 border-amber-300/50' : 'bg-white border border-gray-200/80'
                            }`}
                            style={{ top: `${contextMenu.y}px`, left: `${contextMenu.x}px` }}
                            onClick={(e) => e.stopPropagation()}
                        >
                            {/* Admin badge and actions */}
                            {isAdmin && (
                            ...
                            <button onClick={(e) => { e.stopPropagation(); handleReplyMessage(contextMenu.messageId); }} className="w-full px-4 py-3 text-left text-sm md:text-base hover:bg-gradient-to-r hover:from-indigo-50 hover:to-purple-50 transition-all duration-200 flex items-center space-x-3 text-gray-700 hover:text-indigo-600 active:scale-[0.98]">
                                <svg className="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6" /></svg>
                                <span className="font-medium">Reply</span>
                            </button>

                            <button onClick={(e) => { e.stopPropagation(); handleEditMessage(contextMenu.messageId); }} className={`w-full px-4 py-3 text-left text-sm md:text-base transition-all duration-200 flex items-center space-x-3 active:scale-[0.98] ${isAdmin ? 'hover:bg-gradient-to-r hover:from-amber-50 hover:to-orange-50 text-gray-700 hover:text-amber-700' : 'hover:bg-gradient-to-r hover:from-indigo-50 hover:to-purple-50 text-gray-700 hover:text-indigo-600'}`}>
                                <svg className="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                                <span className="font-medium">Edit{isAdmin ? ' (Admin)' : ''}</span>
                            </button>

                            <button onClick={(e) => { e.stopPropagation(); handleCopyMessage(contextMenu.messageId); }} className="w-full px-4 py-3 text-left text-sm md:text-base hover:bg-gradient-to-r hover:from-indigo-50 hover:to-purple-50 transition-all duration-200 flex items-center space-x-3 text-gray-700 hover:text-indigo-600 active:scale-[0.98]">
                                <svg className="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>
                                <span className="font-medium">Copy</span>
                            </button>

                            <button onClick={(e) => { e.stopPropagation(); handleMessageSelect(contextMenu.messageId); }} className="w-full px-4 py-3 text-left text-sm md:text-base hover:bg-gradient-to-r hover:from-indigo-50 hover:to-purple-50 transition-all duration-200 flex items-center space-x-3 text-gray-700 hover:text-indigo-600 active:scale-[0.98]">
                                <svg className="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" /></svg>
                            ...
                            {isAdmin && (
                                <>
                                    ...
                                    <button onClick={(e) => { e.stopPropagation(); const message = messages.find(m => m.id === contextMenu.messageId); if (message) { console.log('Admin viewing message details:', message); alert(`Message Details:\nID: ${message.id}\nSender: ${message.senderId}\nTime: ${new Date(message.timestamp).toLocaleString()}\nStatus: ${message.status}`); } setContextMenu(null); }} className="w-full px-4 py-3 text-left text-sm md:text-base hover:bg-gradient-to-r hover:from-amber-50 hover:to-orange-50 transition-all duration-200 flex items-center space-x-3 text-amber-700 hover:text-amber-800 active:scale-[0.98]">
                                    ...
                                    <button onClick={(e) => { e.stopPropagation(); const message = messages.find(m => m.id === contextMenu.messageId); if (message && socket?.current) { const reason = prompt('Please provide a reason for flagging this message:'); if (reason && reason.trim()) { socket.current.emit('adminFlagMessage', { messageId: contextMenu.messageId, flaggedBy: currentUser.id, flagReason: reason.trim(), conversationId: message.conversationId, isAdmin: isAdmin }); console.log('🚩 Admin flagging message:', contextMenu.messageId); } else if (reason !== null) { alert('Flag reason is required'); } } setContextMenu(null); }} className="w-full px-4 py-3 text-left text-sm md:text-base hover:bg-gradient-to-r hover:from-amber-50 hover:to-orange-50 transition-all duration-200 flex items-center space-x-3 text-amber-700 hover:text-amber-800 active:scale-[0.98]">
                                        <svg className="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 21v-4m0 0V5a2 2 0 012-2h6.5l1 1H21l-3 6 3 6h-8.5l-1-1H5a2 2 0 00-2 2zm9-13.5V9" /></svg>
                            ...
                            <button onClick={(e) => { e.stopPropagation(); handleDeleteMessage(contextMenu.messageId); }} className={`w-full px-4 py-3 text-left text-sm md:text-base hover:bg-red-50 transition-all duration-200 flex items-center space-x-3 text-red-600 hover:text-red-700 active:scale-[0.98] ${isAdmin ? 'font-semibold' : ''}`}>
                                <svg className="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
            ...
            {/* Ultra Modern Input Area with Glass Effect (sticky) */}
            <div className="relative z-10 bg-white/90 backdrop-blur-xl border-t border-gray-200/50 shadow-2xl safe-bottom">
                <div className="max-w-full px-3 sm:px-5 py-3">
                    {/* Reply Preview */}
                    {replyingTo && (
                        <div className="mb-3 p-3 bg-gradient-to-r from-indigo-50 to-purple-50 border-l-4 border-indigo-500 rounded-lg flex items-start justify-between animate-fadeIn">
                            <div className="flex-1">
                                <div className="flex items-center space-x-2 mb-1">
                                    <svg className="w-4 h-4 text-indigo-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6" /></svg>
                                    <span className="text-xs font-semibold text-indigo-600">Replying to {replyingTo.senderId === currentUser.id ? 'yourself' : activeConversation.user.name}</span>
                                </div>
                                <p className="text-sm text-gray-600 truncate">{replyingTo.text}</p>
                            </div>
                            <button onClick={handleCancelReply} className="ml-2 p-1 text-gray-400 hover:text-gray-600 transition-colors">
                    ...
                    {imagePreview && (
                        <div className="relative inline-block mb-3 sm:mb-4 group">
                            ...
                            <button onClick={cancelImage} className="absolute -top-2 -right-2 p-2 text-white bg-gradient-to-r from-red-500 to-pink-500 rounded-full hover:from-red-600 hover:to-pink-600 shadow-lg transition-all hover:scale-110 active:scale-95" aria-label="Remove image">
                    ...
                    <form onSubmit={handleSend} className="flex items-end space-x-3">
                        ...
                        {/* Attachment Button */}
                        <button type="button" onClick={() => fileInputRef.current?.click()} className="flex-shrink-0 p-3 text-gray-600 rounded-2xl bg-gradient-to-br from-gray-50 to-gray-100 hover:from-indigo-50 hover:to-purple-50 hover:text-indigo-600 border border-gray-200 hover:border-indigo-200 transition-all duration-300 hover:scale-105 active:scale-95 shadow-sm hover:shadow-md">
                            <PaperclipIcon className="w-5 h-5" />
                        </button>

                        {/* Text Input with Glass Effect */}
                        <div className="flex-1 relative">
                            <input type="text" value={inputText} onChange={(e) => setInputText(e.target.value)} placeholder="Type your message..." className="w-full px-4 py-3 text-sm sm:text-base bg-gradient-to-br from-gray-50 to-gray-100 border-2 border-gray-200 rounded-2xl focus:outline-none focus:border-indigo-400 focus:from-white focus:to-indigo-50 transition-all duration-200 placeholder-gray-400 shadow-sm focus:shadow-md" />
                        </div>

                        {/* Send Button with Gradient Animation */}
                        <button type="submit" className="flex-shrink-0 p-3.5 text-white bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 rounded-2xl hover:from-indigo-700 hover:via-purple-700 hover:to-pink-700 focus:outline-none focus:ring-4 focus:ring-indigo-300 shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 group" disabled={!inputText.trim() && !imageFile}>
